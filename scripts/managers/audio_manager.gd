...
func _load_resource(resource_key: String, paths: Dictionary) -> AudioStream:
	var cache_key: String = "%s_%d" % [resource_key, paths.hash()]
	if _cache.has(cache_key):
		return _cache[cache_key]

	if not paths.has(resource_key):
		push_warning("[AudioManager] Recurso de audio desconocido: key='%s'" % resource_key)
		return null

	var path: String = paths[resource_key]
	if not ResourceLoader.exists(path):
		push_warning("[AudioManager] Archivo de audio inexistente: key='%s' path='%s'" % [resource_key, path])
		return null

	var stream = load(path) as AudioStream
	if stream == null:
		push_warning("[AudioManager] No se pudo cargar audio: key='%s' path='%s'" % [resource_key, path])
		return null
